...
void main() {
  ...
  Widget button(String label) => Button(
    onPressed: () {},
    center: Text(label),
  ...
  testWidgets('the run rounds its outside and squares every seam', (
    ...
    expect(
      radiusOf(tester, 'a'),
      BorderRadius.only(topLeft: own.topLeft, bottomLeft: own.bottomLeft),
  ...
  testWidgets('neighbors share their edge rather than drawing two', (
    tester,
  ) async {
    await pump(
      tester,
    ...
    final buttons = [
      for (final label in ['a', 'b', 'c'])
        tester.getRect(
          find.ancestor(of: find.text(label), matching: find.byType(Button)),
        ),
    ];
    expect(
      buttons[1].left,
      buttons[0].right - 1,
      reason: 'one hairline of overlap',
    );
  ...
  testWidgets('vertical stacks the run, rounding its top and bottom', (
    ...
    await pump(
      tester,
      ButtonGroup(axis: Axis.vertical, children: [button('a'), button('b')]),
    );
    ...
    final a = tester.getRect(
      find.ancestor(of: find.text('a'), matching: find.byType(Button)),
    );
    final b = tester.getRect(
      find.ancestor(of: find.text('b'), matching: find.byType(Button)),
    );
